...
def run_system(my_id):
    ...
    dictionary['times' + str(my_id)] = times
    dictionary['best_val_scores' + str(my_id)] = best_val_scores
...
parallel = 20
with NestablePool(processes=parallel) as pool:
    results = pool.map(run_system, range(parallel))


for repeat in range(parallel):
    times_all.append(dictionary['times' + str(repeat)])
    best_val_scores_all.append(dictionary['best_val_scores' + str(repeat)])
