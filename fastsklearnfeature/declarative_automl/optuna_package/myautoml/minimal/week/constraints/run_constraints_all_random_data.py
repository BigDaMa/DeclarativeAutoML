...
def run_AutoML(trial):
    ...
    try:
        if fairness_limit > 0.0:
            ...
            task_id = trial.user_attrs['dataset_id_fair']
...
def run_AutoML_global(trial_id):
    ...
    if 'dataset_id' in mp_glob.my_trials[trial_id].params:
        ...
        dataset_name = 'normal_' + str(mp_glob.my_trials[trial_id].user_attrs['dataset_id'])
    else:
        #dataset_name = 'fair_' + str(mp_glob.my_trials[trial_id].params['dataset_id_fair'])
        dataset_name = 'fair_' + str(mp_glob.my_trials[trial_id].user_attrs['dataset_id_fair'])
...
def sample_and_evaluate(my_id1):
    ...
    try:
        ...
        dataset_name = ''
        if 'dataset_id' in best_trial.user_attrs:
            dataset_name = 'normal_' + str(best_trial.user_attrs['dataset_id'])
        else:
            dataset_name = 'fair_' + str(best_trial.user_attrs['dataset_id_fair'])
