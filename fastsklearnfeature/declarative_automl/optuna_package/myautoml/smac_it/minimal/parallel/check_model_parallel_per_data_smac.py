...
from fastsklearnfeature.declarative_automl.optuna_package.myautoml.smac_it.minimal.parallel.utils import query_model
import numpy as np
import copy
...
for test_holdout_dataset_id in [args.dataset]:
    ...
    mp_global.metafeature_values = data2features(X_train_hold, y_train_hold, categorical_indicator_hold)
    ...
    mp_global.new_model = CustomRandomForest(**conf)
    mp_global.new_model.rf = model

    mp_global.cs = conf['configspace']
