...
#img = '_p2'
#img = '_new_sampling'
img = '_new_sampling2dfixed#'


import glob

#file_list = glob.glob("/home/neutatz/phd2/picture_progress/all_test_datasets/all_results_cost_sens"+ img +"*.p")
#file_list = glob.glob("/home/neutatz/data/automl_runs/mine_*.p")
file_list = glob.glob("/home/neutatz/data/automl_runs/mine_async_m1*.p")

print(file_list)

#file_list = ['/home/neutatz/data/automl_runs/mine_162.p']

for f in file_list:
    try:
        v = pickle.load(open(f, 'rb'))#test
        #new_dct = pickle.load(open('/home/neutatz/phd2/picture_progress/all_test_datasets/all_results_squared4.p', 'rb'))#test

        k = int(f.split('.')[-2].split('_')[-1])
        #k = int(f.split('.')[-2].split('#')[-1])
        ...
        plt.savefig('/home/neutatz/phd2/picture_progress/all_test_datasets/new_run4/data_' + name + '_' + str(k) + img +'.png')
        plt.clf()
