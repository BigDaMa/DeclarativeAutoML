...
#search_time autosklearn
#new_dct = pickle.load(open('/home/felix/phd2/picture_progress/all_test_datasets/all_results_only_success.p', 'rb'))
#new_dct_autosklearn = pickle.load(open('/home/felix/phd2/picture_progress/all_test_datasets/all_results_auto_sklearn_best.p', 'rb'))

new_dct = pickle.load(open('/home/neutatz/phd2/picture_progress/all_test_datasets/all_results.p', 'rb'))
new_dct_autosklearn = pickle.load(open('/home/neutatz/phd2/picture_progress/all_test_datasets/all_results_auto_sklearn_more_time.p', 'rb'))


print(new_dct_autosklearn)
...
map_constraint2values_autosklearn = {}

counter = 0
for k, v in new_dct.items():
    if k in new_dct_autosklearn and len(new_dct_autosklearn[k]['dynamic']) == len(new_dct[k]['dynamic']):
        ...
        for constraint_v in range(len(v['dynamic'])):
            if not constraint_v in map_constraint2values_static:
                ...
                map_constraint2values_autosklearn[constraint_v] = []
            ...
            my_v = new_dct_autosklearn[k]
            print(my_v)
            my_dyn = my_v['dynamic']
            print(my_dyn)
            print(len(my_dyn))
            print(len(v['dynamic']))
            map_constraint2values_autosklearn[constraint_v].append(np.mean(my_v['dynamic'][constraint_v]))
...
latex += 'AutoSklearn'
for constraint_v in range(counter):
    latex += " & $" + "{:.2f}".format(np.mean(map_constraint2values_autosklearn[constraint_v])) + ' \pm ' + "{:.2f}".format(np.std(map_constraint2values_autosklearn[constraint_v])) + "$"
latex += '\\\\ \n'
