...
for test_holdout_dataset_id in [args.dataset]:
    ...
    random_configs_old = copy.deepcopy(random_configs)

    arr = np.arange(len(random_configs_old))
    np.random.seed(42)
    np.random.shuffle(arr)
    random_configs_old = random_configs_old[arr]


    #for minutes_to_search in [10, 30, 1*60, 5*60, 10*60, 60*60]:#[1, 5]:#range(1, 6):
    for config_fraction in [0.25, 0.5, 0.75]:
    #for minutes_to_search in [10 * 60, 60 * 60]:
        ...
        new_constraint_evaluation_dynamic = ConstraintEvaluation(dataset=test_holdout_dataset_id,
                                                                 constraint={'config_fraction': config_fraction},
                                                                 system_def='dynamic')
