...
for test_holdout_dataset_id in [args.dataset]:
    ...
    model_success_regression = pickle.load(
        open('/home/' + getpass.getuser() + '/data/my_temp/my_great_model_compare_scaled.p', "rb"))
    ...
    for pipeline_size_limit in [3175.0, 4026.22, 6651.52, 8359.48, 16797.0, 32266.96]:
        ...
        new_constraint_evaluation_dynamic = ConstraintEvaluation(dataset=test_holdout_dataset_id,
                                                                 constraint={'pipeline_size_limit': pipeline_size_limit},
                                                                 system_def='dynamic')
        for repeat in range(10):
            ...
            predictions_regression_best_id = np.argmax(model_success_regression.predict(random_configs))
