...
class SpaceGenerator:
    def __init__(self):
        ...
        self.space = EmuKitSpace()

        #generate binary or mapping for each hyperparameter


    def generate_params(self, y):

        class_weighting_p = self.space.generate_cat('class_weighting', [True, False], True)
        custom_weighting_p = self.space.generate_cat('custom_weighting', [True, False], False,
                                                     depending_node=class_weighting_p[0])

        unique_counts = np.unique(y)
        for unique_i in range(len(unique_counts)):
            self.space.generate_number('custom_class_weight' + str(unique_i), 1.0)
        ...
        category_preprocessor = self.space.generate_cat('preprocessor', self.preprocessor_list,
                                                        self.preprocessor_list[0])
        ...
        category_private_classifier = self.space.generate_cat('private_classifier', self.private_classifier_list,
                                                              self.private_classifier_list[0])
        ...
        category_categorical_encoding = self.space.generate_cat('categorical_encoding', self.categorical_encoding_list,
                                                                self.categorical_encoding_list[0])
