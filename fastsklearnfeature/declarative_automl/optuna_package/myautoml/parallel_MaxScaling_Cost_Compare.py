...
def run_AutoML(trial):
    ...
    comparison = np.average(dynamic_values) / np.average(static_values)
...
def optimize_uncertainty(trial):
    ...
    #objective = (uncertainty / calculate_max_std(model.n_estimators, min_value=0.0, max_value=1.0)) / float(search_time / float((mp_glob.total_search_time*60)))
...
for result_p in results:
    for f_progress in range(len(result_p['feature_l'])):
        if result_p['target_l'][f_progress] >= 0.0:
