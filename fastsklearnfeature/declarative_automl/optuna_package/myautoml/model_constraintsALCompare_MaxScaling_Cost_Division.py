...
def run_AutoML(trial, X_train=None, X_test=None, y_train=None, y_test=None, categorical_indicator=None):
    ...
    average_dynamic = float(np.average(dynamic_params))
    average_static = float(np.average(static_params))


    if average_dynamic == 0.0:
        return 0.0, search
    if average_static == 0.0:
        return 1.0, search

    # we devide configuration score by default score
    quotient = average_dynamic / average_static
    # and limit it to a factor of 2
    if quotient > 2.0:
        quotient = 2.0
    # and max-scale it to have uniform uncertainty
    quotient /= 2.0

    return quotient, search
...
def optimize_uncertainty(trial):
    try:
        ...
        # increase the diversity of datasets
